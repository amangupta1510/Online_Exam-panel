...
<div class="d-sm-flex justify-content-between align-items-center mb-4 pt-2">
    <h3 class="text-dark mb-0"><a class="temp_name" data-temp_name="{{$users[0]->name}}">{{$users[0]->name}}&nbsp;&nbsp;&nbsp;&nbsp;<i class="glyphicon glyphicon-pencil edit_temp" data-temp_name="{{$users[0]->name}}" style="color: #ff9933;font-size: 15px;"></i></a>&nbsp;&nbsp;&nbsp;<a class="btn btn-primary add-subject" style="padding: 6px 8px;font-size: 13px;color:#fff;">Add subject</a>&nbsp;&nbsp;<a class="btn btn-danger create-paper" style="padding: 6px 8px;font-size: 13px;color:#fff;">Update Template</a></h3>
</div>
<div class="row add_subject_div">
    ...
    <?php $no=0; $color = array("nothing", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success", "danger" , "warning" , "primary" , "success"); ?>
    @foreach(json_decode($users[0]->structure) as $use)
    @foreach($use->pattern as $user)
    @foreach($user->pattern as $u)
    <?php $subject_code = $u->subject_code;?>
    @endforeach
    <div class="col-md-6 col-xl-3 mb-4 subject_div" data-subject="{{$subject_code}}">
        <div class="card shadow border-left-{{$color[$subject_code]}}">
            <div class="card-body">
                <div class="row align-items-center no-gutters">
                    <div class="col mr-2">
                        <div class="text-{{$color[$subject_code]}} font-weight-bold text-s title" id="title_{{$subject_code}}" data-subject="{{$subject_code}}" data-subject_name="{{$user->subject}}"><span>Subject : {{$user->subject}}</span></div>
                        <div class="text-dark mb-3 text-xs description" id="description_{{$subject_code}}" data-subject="{{$subject_code}}" data-pattern="{{urlencode(json_encode($user->pattern))}}"></div>
                        <div class="text-dark mb-0 text-xs font-weight-bold"><a style="font-size: 14px; max-width: 70%; float: left;"><i class="glyphicon glyphicon-trash pt-1 delete_subject" data-subject="{{$subject_code}}" data-subject_name="{{$user->subject}}" style="color: #ff5c33;font-size: 15px;"></i>&nbsp;&nbsp;&nbsp;&nbsp;<i class="glyphicon glyphicon-pencil edit_subject" id="edit_subject_{{$subject_code}}" data-subject="{{$subject_code}}" data-subject_name="{{$user->subject}}" style="color: #ff9933;font-size: 15px;"></i></a><a style="float: right; font-size: 11px; padding: 3px 6px; color: #fff;" class="btn btn-success add_question" id="add_question_{{$subject_code}}" data-subject="{{$subject_code}}" data-pattern="{{urlencode(json_encode($user->pattern))}}" data-toggle="modal" data-target="#select_question">Add Question&nbsp;<i style="font-size: 11px;" class="fa fa-plus"></i></a></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    $('.subject_div').each(function() {
        ...
        for (var i = 0; i < array.length; i++) {
            if (array[i].question != null && array[i].question > 0) {
                ...
                $('#description_' + id).empty().html(desc + '<a>' + type + ' <b>' + array[i].question + '</b> Questions. <b>+ve ' + array[i].positive + ' , -ve ' + array[i].negative + ' marks</b></a><br>');
            }
        }
    });
}
...
$("body").delegate(".create-paper", "click", function() {
    ...
    $(".spinner").hide();
    $(".btn-cancel").show();
    $('.socket_title').text('Confirm Update Paper');
    ...
    var html = $('.socket_body').html();
    $('.socket_body').empty().html(html + '<br><br><a style="color:#fff;" class="btn btn-danger mt-1 create_paper_confirm" style="margin:10px;" data-sub_count="' + sub_count + '" data-ques_count="' + ques_count + '" data-marks="' + marks + '">Update</a></div>');
    $("#socket").show("closed");
});
$("body").delegate(".create_paper_confirm", "click", function() {
    ...
    $.ajax({
        type: 'POST',
        url: '{{ route('
        admin - edit_custom_paper_templates_submit ') }}',
        data: {
