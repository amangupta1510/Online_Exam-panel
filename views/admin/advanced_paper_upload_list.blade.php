...
<div class="limiter">
    <div class="title">
        <div class="searchBox">
            <h4><b>Upload Paper</b>@if(Auth::user()->admin=='yes') <a href="{{ route('admin-adv_paper_upload') }}"> <i class="glyphicon glyphicon-2x glyphicon-upload" style="color: skyblue"></i></a>@endif
                <div class="searchForm">
                    <form action="{{route('admin-advanced_paper')}}" method="get">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr class="user{{$user->id}}">
                            ...
                            <td class="column3"><a style="color: #fff" class="btn btn-primary" href="{{ route('admin-advanced_paper_ques_upload',['id'=>$user->id]) }}">Upload paper</a></td>
                            <td class="column4"><a style="color: #fff" class="btn btn-danger" href="{{ route('admin-advanced_paper_ans_upload',['id'=>$user->id]) }}">Upload Ans.</a></td>
                            {{-- <td class="column5"><a style="color: #fff" class="btn btn-info" href="{{ route('admin-advanced_paper_update_result_page',['id'=>$user->id]) }}">Update</a></td> --}}
                            <td class="column5"><a style="color: #fff" class="btn btn-warning" href="{{ route('admin-advanced_paper_publish',['id'=>$user->id]) }}">Publish</a></td>
                            <td class="column6"><a title="Syllabus View" id="syllabus-button" id="syllabus-button" data-toggle="modal" data-target="#syllabus" data-id="{{$user->pname}}">
                                    <i class="fa fa-book" style="color: #ff9933"></i></a>&nbsp;&nbsp;&nbsp;<a title="Paper Pdf Download" class="pdf_download" data-id="{{$user->id}}" data-pname="{{$user->pname}}" data-type="{{$user->type}}"><i class="fa fa-file-pdf-o" style="color: #e91e63"></i></a>&nbsp;&nbsp;&nbsp;<a title="Edit Paper Details" href="{{ route('admin-editadvancedpaperpage',['id'=> $user->id]) }}"><i class="fa fa-edit" style="color: #ff9933"></i></a></td>
                            ...
                            <td class="column8"><a title="Rank List PDF" href="{{ route('admin-advanced_paper_summary',['id'=>$user->id]) }}"><i style="color: #fd6e70" class="glyphicon glyphicon-download"></i></a>&nbsp;&nbsp;<a title="Move Paper To Test Series" class="btn btn-secondary move_paper" style="font-size: 12px;color:#fff;padding: 3px 4px;" data-id="{{$user->id}}" data-name="{{$user->pname}}" data-type="advanced">move</a></td>
                            <td class="column9"> <a title="Move To Recycle Bin" id="delete-button" data-toggle="modal" data-target="#delete" data-name="{{$user->pname}}" data-id="{{$user->id}}">
                                    <i class="glyphicon glyphicon-trash" style="color: #ff5c33"> </i> </a>@if(Auth::user()->admin=='yes')&nbsp;<a title="Download Paper Pattern + Data" href="{{ route('admin-adv_paper_download',['id'=>$user->id]) }}"> <i class="glyphicon glyphicon-download" style="color: #4CAF50"></i> </a>@endif</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
    ...
    $(document).on('click', '.pagination a', function(e) {
        ...
        var img = "{{ route('admin-advanced_paper_page',['s'=>':year','page'=>':page']) }}";
    ...
    $('#p_delete-student').click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route('
            admin - advanced_paper_update_result ') }}',
            data: {
                '_token': $('input[name=_token]').val(),
                'id': $('#p_delete-id').text(),
            },
    ...
    $(document).on('click', '#show-button', function() {
        ...
        loading.style.display = '';
        var id = $(this).data('id');
        var img = "{{ route('admin-adv_image',['id'=>':id']) }}";
    ...
    $(document).on('click', '#notification-button', function() {
        ...
        $('.msg-button').data('channel', 'advanced_paper_' + $(this).data('id'))
        var img = "{{ route('admin-paper_sockets',['channel'=>':channel','event'=>'online']) }}";
        var img = img.replace('%3Achannel', 'advanced_paper_' + $(this).data('id'));
        var img = img.replace('&amp;', '&');
        $.get(img, function(data) {
            $("#loading").fadeOut(500);
        })
    ...
    function socket(i) {
        ...
        channel.listen('.online', function(data) {
            ...
            $('#notification_table').append('<li><div><a style="display:inline-block; white-space: nowrap; width: 140px; overflow: hidden;text-overflow: ellipsis;">' + data.channel.message.sname + '</a><div style="float:right;"><a style="font-size:14px; border:1px;">remain ' + parseInt(data.channel.message.timeleft / 60) + ' mins</a>&nbsp;&nbsp;<a class="msg-button" data-channel="advanced_paper_' + i + '" data-title="Message to ' + data.channel.message.sname + '" data-event="message_student_' + data.channel.message.sid + '"><i class="fa fa-comment-o" style="color: #ff5c33; font-size: 16px;"></i></a>&nbsp;&nbsp;<a class="msg-button" data-channel="advanced_paper_' + i + '"  data-title="Relaod paper of user (' + data.channel.message.sname + ')" data-event="reload_student_' + data.channel.message.sid + '"><i class="fa fa-refresh" style="color: #ff5c33; font-size: 16px;"></i></a>&nbsp;&nbsp;<a class="msg-button" data-channel="advanced_paper_' + i + '" data-title="Submit paper of user (' + data.channel.message.sname + ')" data-event="submit_student_' + data.channel.message.sid + '"><i class="fa fa-check-square-o" style="color: #ff5c33; font-size: 16px;"></i></a></div></li>');
        });
    }
